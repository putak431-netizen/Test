...
PANEL_CONFIG = {
    "enabled": False,
    "api_url": "",
    "api_key": "",  # فرمت: username|password
    "test_config": {
        "volume": 50,  # مگابایت
        "expiry_hours": 3,  # ساعت
        "enabled": True
...
def reset_webhook_and_clean():
    """پاک کردن webhook و بستن درخواست‌های قبلی"""
    try:
        url = f"https://api.telegram.org/bot{TOKEN}/deleteWebhook"
        response = requests.post(url, json={"drop_pending_updates": True})
        logger.info(f"Webhook delete: {response.json()}")
        
        url = f"https://api.telegram.org/bot{TOKEN}/close"
        response = requests.post(url)
        logger.info(f"Close: {response.json()}")
        
        time.sleep(2)
        
        url = f"https://api.telegram.org/bot{TOKEN}/setWebhook"
        response = requests.post(url, json={"url": ""})
        logger.info(f"Webhook clear: {response.json()}")
...
def get_panel_session(api_url, username, password):
    """گرفتن سشن برای اتصال به پنل سنا (3x-UI)"""
    session = requests.Session()
    
    # روش اول: لاگین با endpoint استاندارد
    try:
        # برای نسخه جدید 3x-UI
    ...
    # روش دوم: برای نسخه قدیمی‌تر
    try:
        login_res = session.post(
            f"{api_url}/panel/login",
            json={"username": username, "password": password},
            timeout=30
        )
        if login_res.status_code == 200:
    ...
    return None, "خطا در اتصال به پنل - وضعیت لاگین ناموفق"

def create_account_on_panel(plan, account_name, db_data):
    """ساخت اکانت در پنل سنا (3x-UI)"""
    panel = db_data.get("panel_config", {})
    if not panel.get("enabled"):
        return None, "پنل متصل نیست"
    
    try:
        api_credentials = panel.get('api_key', '')
        if '|' not in api_credentials:
            return None, "فرمت API Key اشتباه است. باید به صورت username|password باشد"
        
        username, password = api_credentials.split('|', 1)
        api_url = panel.get('api_url', '').rstrip('/')
        
        # گرفتن سشن
        session, error = get_panel_session(api_url, username, password)
        if not session:
            return None, error
        ...
        inbounds_res = session.get(f"{api_url}/panel/api/inbounds/list", timeout=30)
        
        if inbounds_res.status_code != 200:
            return None, "خطا در دریافت لیست اینباندها"
        ...
        for inbound in inbounds:
            if inbound.get('enable', True):
                active_inbound_id = inbound.get('id')
                break
        
        if not active_inbound_id:
            return None, "هیچ اینباند فعالی یافت نشد"
        
        # ساخت ایمیل منحصر به فرد برای کاربر
        client_email = f"{account_name}_{datetime.now().strftime('%Y%m%d%H%M%S')}".replace(' ', '_')
        
        add_payload = {
            "id": active_inbound_id,
            "settings": json.dumps({
                "clients": [{
                    "email": client_email,
                    "totalGB": volume_gb,
        ...
        if add_res.status_code == 200:
            ...
            if result.get('success'):
                ...
                config_res = session.post(
                    f"{api_url}/panel/api/inbounds/getClientConfig",
                ...
                if config_res.status_code == 200:
                    ...
                    if config_data.get('success'):
                        config_url = config_data.get('obj', '')
                        if config_url:
                            return config_url, None
                
                # روش جایگزین: ساخت لینک سابسکریپشن دستی
                return f"{api_url}/sub/{client_email}", None
...
def create_test_account(db_data):
    """ساخت اکانت تست در پنل سنا (3x-UI)"""
    panel = db_data.get("panel_config", {})
    ...
    try:
        api_credentials = panel.get('api_key', '')
        if '|' not in api_credentials:
            return None, "فرمت API Key اشتباه است"
        ...
        # تبدیل مگابایت به گیگابایت (تست حجم کم)
        volume_mb = test_config.get("volume", 50)
        volume_gb = round(volume_mb / 1024, 2)
        expiry_hours = test_config.get("expiry_hours", 3)
        expiry_time = int((datetime.now() + timedelta(hours=expiry_hours)).timestamp() * 1000)
        
        # دریافت لیست اینباندها
        inbounds_res = session.get(f"{api_url}/panel/api/inbounds/list", timeout=30)
        
        if inbounds_res.status_code != 200:
            return None, "خطا در دریافت اینباندها"
        ...
        if not active_inbound_id:
            return None, "اینباند فعال یافت نشد"
        
        client_email = f"test_{datetime.now().strftime('%Y%m%d%H%M%S')}_{random.randint(1000,9999)}"
    ...
    try:
        ...
        if str(uid) == str(ADMIN_ID):
            ...
            if text == '🔌 تنظیمات پنل':
                ...
                api_url = db['panel_config'].get('api_url', 'تنظیم نشده')
                api_key = db['panel_config'].get('api_key', 'تنظیم نشده')
                if api_key != 'تنظیم نشده':
                    api_key = '********'
                update.message.reply_text(
                    f"🔌 وضعیت پنل: {status}\nآدرس API: {api_url}\nAPI Key: {api_key}",
            ...
            if text == '🔗 تنظیم API':
                ...
                update.message.reply_text("آدرس پنل را بفرستید:\nمثال: https://yourdomain.com یا http://IP:PORT", reply_markup=back_btn())
                return

            if text == '🔑 تنظیم API Key':
                user_data[uid] = {'step': 'set_api_key'}
                update.message.reply_text("نام کاربری و رمز پنل را به فرمت username|password بفرستید:\nمثال: admin|123456", reply_markup=back_btn())
                return
            ...
            if step == 'set_api_key':
                if '|' in text:
                    ...
                    save_db(db)
                    update.message.reply_text("✅ اطلاعات ورود به پنل ذخیره شد", reply_markup=admin_menu())
                else:
                    update.message.reply_text("❌ فرمت اشتباه! باید به صورت username|password باشد")
                user_data[uid] = {}
                return
            ...
            if step == 'set_test_volume':
                try:
                    ...
                    if volume > 0:
                        ...
                        update.message.reply_text("❌ عدد بزرگتر از صفر وارد کنید")
                except:
