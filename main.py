...
# --- توکن و آیدی ادمین ---
TOKEN = '8298942850:AAGCIOaijocHczkssq1Sqb6c8auIMp5sN9o'
ADMIN_ID = 5993860770
...
def get_user_info(uid, context):
    """گرفتن اطلاعات کاربر شامل یوزرنیم و نام"""
    try:
...
def apply_discount(price, discount_code, db):
    """اعمال کد تخفیف"""
    if discount_code in db.get("discount_codes", {}):
        code_data = db["discount_codes"][discount_code]
...
def create_account_on_panel(plan, account_name, db):
    """ساخت اکانت در پنل"""
    ...
    try:
        ...
        # درخواست به API پنل (فرمت استاندارد Marzban)
        headers = {
...
def handle_msg(update, context):
    try:
        ...
        if str(uid) == str(ADMIN_ID):
            ...
            # دریافت کانفیگ (برای تست و خرید تایید شده)
            if step == 'send_config':
        ...
        # دریافت نام برای خرید (با کد تخفیف)
        if step == 'wait_name':
...
def handle_cb(update, context):
    try:
        ...
        # تایید و رد فیش (ادمین)
        elif query.data.startswith("approve_"):
            if str(uid) == str(ADMIN_ID):
                target_uid = query.data.split("_")[1]
                plan_data = json.loads(query.data.split("_")[2])
